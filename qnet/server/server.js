...
app.post("/api/person", (req, res) => {
  const { name, sexuality, gender, age, location, questions, email, password } = req.body;

  if (!name || !sexuality || !gender || !age || !location || !questions || !email || !password) {
    return res.status(400).send({ success: false, message: "All fields are required." });
  }

  const newPerson = new Person({ name, sexuality, gender, age, location, questions, email, password });
